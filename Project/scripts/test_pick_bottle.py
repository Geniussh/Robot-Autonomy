...
def pick_up_bottle(shots, initial_pose, fa, center, azure_kinect_depth_image, azure_kinect_intrinsics, azure_kinect_to_world_transform, grasp_offset, cup_world):
    ...
    object_center_pose = initial_pose.copy()
    object_center_pose.rotation = object_center_pose.rotation @ np.array([[0,1,0], [0,0,-1], [-1,0,0]])
    # TODO: hardcoded_offset
    object_center_pose.translation = [object_center_point_in_world[0], object_center_point_in_world[1] + grasp_offset, object_z_height]
    ...
    #Bottle
    fa.goto_pose(object_center_pose, 7, force_thresholds=[10, 10, 10, 10, 10, 10])
    ...
    cup_intermediate_pose_vertical = fa.get_pose()

    #Move to cup intermediate
    for _ in range(shots):
        intermediate_height = 0.3
        cup_intermediate_pose = cup_intermediate_pose_vertical.copy()
        cup_intermediate_pose.translation = [cup_world[0] - 0.12, cup_world[1], intermediate_height]
...
if __name__ == '__main__':
    ...
    for _ in range(2):
        ...
        cup_pose = cup_pose * R
        fa.goto_pose(cup_pose, 15, force_thresholds=[10, 10, 20, 10, 10, 10], buffer_time=5)

...
def mix(fa):
    ...
    T_delta = RigidTransform(
        translation=np.array([0, radius, 0]),
                            from_frame=p0.from_frame, to_frame=p0.from_frame)

    fa.goto_pose(p0 * T_delta)
    ...
    x, y, z = p0.translation
    radius = 0.025
    pose_traj = []
    for theta in np.arange(0, 1080 + 1080 // len(ts), 1080 // len(ts)):
        T_delta = RigidTransform(
            translation=np.array([radius * np.sin(np.deg2rad(theta)), radius * np.cos(np.deg2rad(theta)), 0]),
                                from_frame=p0.from_frame, to_frame=p0.from_frame)
        pose_traj.append(p0 * T_delta)
...
if __name__ == "__main__":
    ...
    # p1 = p0.copy()
    T_delta = RigidTransform(
        translation=np.array([0, 0.025, 0]),
                            from_frame=p0.from_frame, to_frame=p0.from_frame)
    # p1 = p1 * T_delta
    # fa.goto_pose(p1)

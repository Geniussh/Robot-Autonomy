...
def find_drink_old(rgb_image, lower, upper, border):
    ...
    mask = cv2.erode(mask, np.ones((5,5), np.uint8), iterations=1)
    mask = cv2.dilate(mask, np.ones((5,5), np.uint8), iterations=1)
    _, cnts, hie = cv2.findContours(mask.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
...
def find_drink(rgb_image, lower, upper, border):
    ...
    if childc_i == -1:
        ...
        _, hole_cnts, _ = cv2.findContours(hole_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        if len(hole_cnts) > 0:
...
if __name__ == '__main__':
    ...
    find_drink(rgb_image, bLower, bUpper, border)
